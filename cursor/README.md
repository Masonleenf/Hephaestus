...
- `plugin/` — Cursor plugin: `/hephaestus`, `/hephaestus-network` commands +
  the `hephaestus-network` skill (slash-invokable as `/hephaestus-network` via
  skills too) + the routing rule.
...
cp cursor/plugin/commands/*.md ~/.cursor/commands/
