...
description: "Use when the user types /hep-storm, mentions @Hephaestus storm <goal>, or asks to drive a loop-worthy goal (apps, sites, agents, automations, debugging, multi-step research, data/report generation) all the way to a verified result. Runs the force-robust Stormbreaker Loop: route the goal to real Agentlas specialists, materialize a dependency-ordered pipeline fabric, and execute it verifier-first without stalling, running away, or claiming false success."
---
...
gives you the verified plan; you carry it out with your own tools. Resolve the
runner with the first executable hit, falling back to the runtime cache:
...
# Agentlas sign-in: opens the browser only when no valid local sign-in exists;
# a saved sign-in is silently reused. Set HEPHAESTUS_AUTH_AUTOPOPUP=0 for CI/headless.
