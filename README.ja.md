...
```text
このワークスペースに Hephaestus Agentlas メタエージェントをセットアップして。GitHub リポジトリはこちら:
...
- **Automation output は変更なし。** fallback は chat command/prompt surface のみで、JSON を返す shell CLI output には混ぜません。
