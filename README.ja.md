...
Cursor）で使う 3 つのコマンドを教えて: /hephaestus-build,
/hephaestus-network, /hephaestus-cloud。失敗したらエラーを読んで直し、
もう一度試して。
```

終わったら 3 つだけ覚えれば十分です。作成は `/hephaestus-build`、Hub から
借りるときは `/hephaestus-network`、自分の Agentlas Cloud に保存した agent は
`/hephaestus-cloud`。古い `/hephaestus` も作成 alias として残ります。
...
Codex がすでに開いている場合は、新しいチャットを開始して `/plugins` で plugin が見えることを確認してください。インストール後は `/prompts:hephaestus-build ontology` を実行します。
