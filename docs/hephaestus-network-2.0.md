...
standardized routing cards, fall back to the Agentlas Hub only with your
approval, and keep all user/project memory on your machine.
...
- Routing receipts for every decision, and approval gates for file writes,
  cloud calls, payments, publishing, deletion, private data export, and
  external tools.
...
1. **Local first.** Explicit commands → project overrides
   ...
   Hub, and only with your approval.
...
hephaestus network grant <capability> --target <id> --scope per_call|session|project|global
...
| `refuse` | loop guard or privacy block; the reason is in `reasons` |
