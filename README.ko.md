...
- **메모리 관계 그래프.** durable memory를 그래프로: candidate ticket을 타입드
  엣지(similar/supersedes/contradicts)로 잇고, 근접 중복을 자동 탐지하며, 교체
  시 supersedes 엣지를 남겨 새 학습이 옛 학습을 소리 없이 덮어쓰지 않게 합니다.
  명령: `ontology memory dedup | graph | link`.
- **Stormbreaker 런 저널.** 장기 작업이 단계 원장을 남겨 중단돼도 처음부터가
  아니라 안 끝난 단계부터 재개합니다. 루프는 hard-stop, 하다 만 단계는 재시도용
  으로 봉인. 명령: `stormbreaker journal status | repair | verify`.
- **검증-우선 게이트 + 확인 인터럽트.** 단계가 "어떻게 검증할지"를 먼저 선언하고
  결과를 기록해, 검증 안 된 완료는 따로 표시합니다. 애매하면 멈춰 되묻고,
  `stormbreaker journal gate`가 하나의 ok/blockers 판정으로 검사 통과 전에는
  완료로 안 칩니다.
...
최신 릴리스/설치 안내를 기준으로 진행하고, 내가 쓰는 도구(Claude Code, Codex, Gemini CLI, Antigravity, Cursor)에서 쓸 정확한 세 명령을 알려줘. 실패하면 에러를 읽고 고친 뒤 다시 시도해줘.
```
