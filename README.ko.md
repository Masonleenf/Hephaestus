...
```text
이 워크스페이스에 Hephaestus Agentlas를 설치해줘. 아래 명령을 실행해:
curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.7.6/scripts/install-all-runtimes.sh | bash

끝나면 내가 쓸 세 가지 명령을 알려줘:
1. 에이전트 생성/빌드
2. 공개 Hub 에이전트 빌리기
3. 내 Agentlas Cloud에 저장한 에이전트 사용

그리고 고급 사용자를 위한 두 명령도 알려줘:
4. Cloud와 Hub 후보만 검색하기
5. 내가 지정한 agent slug 직접 호출하기
...
| 할 일 | 명령 | 예시 프롬프트 |
|---|---|---|
| 생성 | `/hephaestus-build` | `/hephaestus-build 쇼피파이 환불용 고객지원 에이전트 만들어줘` |
| 빌리기 | `/hephaestus-network` | `/hephaestus-network 출시 계획을 리서치, 카피, QA, 릴리즈 에이전트로 쪼개서 TF 짜줘` |
| 공유 | `/hephaestus-cloud` | `/hephaestus-cloud 내 클라우드에 저장한 재무 분석 에이전트로 이 보고서 검토해줘` |

### 고급 명령 두 개

| 할 일 | 명령 | 예시 프롬프트 |
|---|---|---|
| 검색 | `/hephaestus-search` | `/hephaestus-search 시장 리포트에 쓸만한 에이전트 찾아줘` |
| 호출 | `/hephaestus-call` | `/hephaestus-call market-researcher, report-writer {시장 리포트 초안 작성}` |

새 설치와 업데이트는 오래된 `/hephaestus` 채팅 명령을 정리하고, 핵심 3개와
고급 2개 명령 표면으로 맞춥니다.
...
```text
/hephaestus-build 고객지원 에이전트 만들어줘
/hephaestus-network 출시 준비에 맞는 에이전트들 빌려와서 TF 짜줘
/hephaestus-cloud 내 Agentlas Cloud에 공유한 에이전트로 처리해줘
```
...
- **명령은 세 개로 압축.** `/hephaestus-build`는 생성,
  `/hephaestus-network`는 Hub 에이전트 빌리기, `/hephaestus-cloud`는 내
  Agentlas Cloud에 공유/보관한 에이전트 사용입니다. `ao`, `network`,
  `cards`, `mcp`는 자동화와 디버그용 내부 표면으로 남습니다.
...
Cursor)에서 쓸 정확한 세 명령을 알려줘. 실패하면 에러를 읽고
고친 뒤 다시 시도해줘.
```

끝나면 도구에서 `/hephaestus-build`, `/hephaestus-network`,
`/hephaestus-cloud` 중 하나를 입력하세요. 직접 명령을 실행하고 싶으면 아래
터미널 **빠른 시작**을 쓰세요.
...
```text
Claude Code: /reload-plugins 실행 후 /hephaestus-build
Codex:       /prompts:hephaestus-build, /prompts:hephaestus-network, /prompts:hephaestus-cloud
Gemini CLI:  /extensions list 또는 /commands list 확인 후 /hephaestus-build
Antigravity: 워크스페이스 다시 열고 /hephaestus-build
```
...
`/hephaestus-build ontology`는 이제 메인 Agent OS 화면이 아니라 Knowledge/Memory 패널입니다. 프로젝트 소스, memory candidate, 재사용 가능한 playbook 재료를 확인할 때 씁니다. 왼쪽 네비게이션, Obsidian 스타일 지식 그래프, 소스 검색, GraphRAG 질문 빌더, Memory Candidate Queue, 복사 가능한 명령어 화면이 포함됩니다.
...
홈 폴더나 옆 프로젝트를 훑지 않습니다. 승인된 회사 문서나 프로젝트 문서는 `.agentlas/ontology-inbox/`에 넣고 `/hephaestus-build ontology`를 다시 실행하면 대시보드가 갱신됩니다.
...
OS 터미널의 Codex CLI 명령은 `codex plugins`가 아니라 `codex plugin` 단수형입니다. Codex 앱 안의 slash command는 `/plugins` 복수형입니다. 플러그인 설치 후 `/prompts:hephaestus-build ontology`를 실행하면 graph, sources, query, memory queue, command 화면이 있는 같은 프로젝트 로컬 대시보드가 만들어집니다.
...
```bash
curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.7.6/scripts/install.sh | bash
...
```powershell
$zip = "$env:TEMP\hephaestus-v0.7.6.zip"
$extract = "$env:TEMP\hephaestus-v0.7.6"
Invoke-WebRequest "https://github.com/agentlas-ai/Hephaestus/archive/refs/tags/v0.7.6.zip" -OutFile $zip
...
Codex 앱 안에서는 `/plugin marketplace add`가 동작하지 않습니다. Codex는 OS 터미널에서 `codex plugin ...`으로 설치하고, 앱 안에서는 `/plugins`로 설치된 플러그인을 확인하세요. 설치 후에는 `/prompts:hephaestus-build ontology`를 실행합니다.

설치 후 세 명령이 안 보이면 해당 프로젝트에서 Claude Code나 Codex를 재시작하세요.
...
| 런타임 | 여는 법 | 그다음 |
|---|---|---|
| Claude Code | OS 터미널에서 `claude` 입력 (또는 데스크톱 앱 실행) | `/hephaestus-build`, `/hephaestus-network`, `/hephaestus-cloud` |
| Codex | OS 터미널에서 `codex` 입력 (또는 Codex 앱) | `/prompts:hephaestus-build`, `/prompts:hephaestus-network`, `/prompts:hephaestus-cloud` |
| Gemini CLI | OS 터미널에서 `gemini` 입력 | `/hephaestus-build`, `/hephaestus-network`, `/hephaestus-cloud` |
| Antigravity | 워크스페이스 열기 | `/hephaestus-build`, `/hephaestus-network`, `/hephaestus-cloud` |

`/hephaestus-build`는 **생성**, `/hephaestus-network`는 공개 Hub 에이전트
**빌리기**, `/hephaestus-cloud`는 내 Agentlas Cloud 에이전트 **공유/사용**입니다.
