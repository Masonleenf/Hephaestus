...
- **첫 설치 뒤 자동 최신화.** `hep-network`, `hep-build`, `hep-search`,
  `hep-call`과 대응 slash/prompt 명령이 하루 최대 한 번 조용한 백그라운드
  업데이트 확인을 시작합니다. 최신 GitHub 릴리즈가 있으면 현재 명령을 막지 않고
  `~/.agentlas/runtime/current`를 갱신합니다.
- **설치된 어댑터도 함께 갱신.** 자동 업데이트는 이미 설치된 Claude, Codex,
  Gemini, Antigravity, Cursor, OpenCode, AgentSkills 명령/스킬 어댑터를 릴리즈
  tarball 내용으로 새로 덮어씁니다. 사용자가 설치하지 않은 런타임은 만들지
  않습니다.
- **필요하면 끌 수 있음.** 자동 업데이트는 기본 ON입니다. 끄려면 Hephaestus 실행
  전에 `HEPHAESTUS_AUTO_UPDATE=0`을 설정하세요. 기존
  `HEPHAESTUS_UPDATE_CHECK=0`도 계속 존중합니다.
