...
Cursor）里应该使用的三个命令：/hephaestus-build、/hephaestus-network、
/hephaestus-cloud。如果出错，请读取报错、修复并重试。
```

完成后，只记住三件事：创建用 `/hephaestus-build`，从 Hub 借 agent 用
`/hephaestus-network`，使用你 Agentlas Cloud 里保存的 agent 用
`/hephaestus-cloud`。旧的 `/hephaestus` 仍然可以作为创建别名使用。
想自己运行命令？请使用下面的 **快速开始**。
...
如果 Codex 会话已经打开，请重新打开一个会话，然后输入 `/plugins` 确认插件可见。安装后运行 `/prompts:hephaestus-build ontology`。
