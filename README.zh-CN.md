...
```text
/hep-network 把会议记录整理成周报
/hep-network 起草新产品发布计划
@Hephaestus 整理并总结这个文件夹里的文档   # 不支持斜杠命令的运行时
hep-network "为这个任务找到合适的智能体"     # 终端
```
...
Cursor）里应该使用的命令：/hep-build、/hep-network、/hep-cloud、
/hep-search、/hep-call、/hep-upload。如果出错，请读取报错、修复并重试。
```
...
如果 Codex 会话已经打开，请重新打开一个会话，然后输入 `/plugins` 确认插件可见。安装后运行 `/prompts:hep-build ontology`。
