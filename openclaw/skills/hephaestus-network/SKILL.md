...
- `route` — report the selected card (`selected.id`,
  ...
  command with the original request.
- `clarify` — ask `clarify_question` with the candidate list and re-route with
  the answer.
- `pipeline` — execute `stages` in order: run the stage card's canonical
  command, save artifacts under
...
- `propose_new` — offer to build a new agent/team via the Hephaestus
  meta-agent.
- `refuse` — explain `reasons` (for example, loop guard). Do not retry
  around it.
